...
class VLTransformerNoRT(nn.Module):
    ...
    def __init__(self, embed_dim=384, depth=6, num_heads=6, mlp_ratio=4,
        ...
        self.v_pos_embed = nn.Embedding(self.num_vtoken, embed_dim)
    ...
    def forward(self, visu_src, visu_mask, lang_src, lang_mask):
        ...
        visu_src = visu_src + self.v_pos_embed.weight.unsqueeze(0)
        lang_src = lang_src + self.l_pos_embed.weight.unsqueeze(0)

        visu_mask = visu_mask.view(batch_size, 1, 1, self.num_vtoken). \
            expand(-1, self.num_heads, -1, -1)
        lang_mask = lang_mask.view(batch_size, 1, 1, self.num_ltoken). \
            expand(-1, self.num_heads, -1, -1)
        
        for block in self.blocks:
            visu_src = block(visu_src, lang_src, visu_mask, lang_mask)
        # reg_hs = rv_src[:, 0, :]
        reg_hs = torch.mean(visu_src, dim=1)
...
def build_vl_module_no_rt(args, num_vtoken, num_ltoken):
    ...
    model = VLTransformerNoRT(embed_dim=embed_dim, depth=args.vl_depth, \

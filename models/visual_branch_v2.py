...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768, depth=12, num_heads=12, \
                 ...
                 vl_loc=[3, 6, 9], reg_out_type='avg_valid'):
        """
        Args:
        ...
        self.reg_token_block = None
        if self.reg_out_type == 'attentive_pool':
            self.reg_block = REGTokenBlock(dim=embed_dim, num_heads=num_heads, 
                                           mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, 
                                           drop=drop_rate, attn_drop=attn_drop_rate, 
                                           drop_path=dpr[i], norm_layer=norm_layer, 
                                           act_layer=act_layer)
    ...
    def forward(self, visu_src, ling_src, visu_mask, ling_mask):
        ...
        if self.reg_out_type == 'avg_valid':
        ...
        elif self.reg_out_type == 'attentive_pool':
            visu_mask_flatten = visu_mask.flatten(1)
            reg_src_list = []
            for bs_ind in range(batch_size):
                this_visu_src  = visu_src[bs_ind]
                ...
                reg_src_list.append(this_reg_src)
            reg_src = torch.cat(reg_src_list, dim=0)
            reg_src = reg_src.unsqueeze(1)
            reg_src = self.reg_block(reg_src, visu_src, ling_src, visu_mask_expanded, ling_mask_expanded)
            reg_src = reg_src.squeeze(1)
        elif self.reg_out_type == 'avg_global':
...
def build_visual_branch_v2(args):
